...
class RequeteRecherche {
   ...
   */
   public function clause_droite_patronyme($pst_patronyme,$pst_variantes) {
      ...
      if (($pst_variantes=='') || ereg('\*',$pst_patronyme))
      {
         if (ereg('\*',$pst_patronyme))
           $st_clause = "like '".str_replace('*','%',$pst_patronyme)."'";
           //$st_clause = "like '".str_replace('*','%',$pst_patronyme)."' ";
         else
           //$st_clause = "= '$pst_patronyme' COLLATE latin1_german1_ci";
     ...
     {   
        ...
        else
        {
           $a_params_precedents=$this->connexionBD->params();   
            ...
            $st_requete = "select prn2.libelle from prenom prn1, prenom prn2 where truncate(prn1.phonex,$gi_precision_prenom)=truncate(prn2.phonex,$gi_precision_prenom) and prn1.libelle=:prn";
