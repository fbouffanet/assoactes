...
function ddes_communes($pi_nb_ddes,$pi_idf_commune,$pi_idf_type_acte)
{
  if ($pi_nb_ddes!=0)
  {
     return "<a href=\"".$_SERVER['PHP_SELF']."?mode=VUE_DEMANDES_COMMUNE&idf_commune=$pi_idf_commune&idf_type_acte=$pi_idf_type_acte\">$pi_nb_ddes</a>";
  }
  else
  {
     return $pi_nb_ddes;
...
function ddes_mois_annee($pi_nb_ddes,$pi_mois,$pi_annee,$pi_idf_type_acte)
{
  if ($pi_nb_ddes!=0)
  {
     return "<a href=\"".$_SERVER['PHP_SELF']."?mode=VUE_DEMANDES_MOIS_ANNNEE&mois=$pi_mois&annee=$pi_annee&idf_type_acte=$pi_idf_type_acte\">$pi_nb_ddes</a>";
  }
  else
  {
     return $pi_nb_ddes;
...
if (isset($gst_ident ))
{
  switch ($gst_mode)
  {
    ...
    case 'VUE_DEMANDES_COMMUNE':
       ...
       $st_requete = "select distinct date_demande,idf_acte from demandes_adherent join adherent on (demandes_adherent.idf_adherent=adherent.idf) where adherent.ident=\"$gst_ident\" and idf_commune=$i_idf_commune and idf_type_acte=$i_idf_type_acte order by date_demande desc";
       //print("Req=$st_requete<br>");
       $a_liste_ddes = $connexionBD->sql_select_multiple($st_requete);       
       $i_nb_ddes = count($a_liste_ddes);
       if ($i_nb_ddes>0)
       {
          $pagination = new PaginationTableau($_SERVER['PHP_SELF'],'num_page_ddes_adht',$i_nb_ddes,NB_LIGNES_PAR_PAGE,DELTA_NAVIGATION,array('Date de la demande','Date de l\'acte','Parties'));
          $a_actes = array();
          ...
          print("<form name=\"DemandesAdherents\" action=\"".$_SERVER['PHP_SELF']."\" method=\"post\">");
          ...
          foreach ($a_liste_ddes as $a_dde)
          {
            ...
            {
              $a_tableau_affichage[] = array($st_date_dde,'&nbsp;',"R&eacute;f&eacute;rence originale de l'acte modifi&eacute;e");
            }  
          } 
    ...
    case 'VUE_DEMANDES_MOIS_ANNNEE':
       ...
       $_SESSION['idf_type_acte']=$i_idf_type_acte;
       $_SESSION['num_page_ddes_adht']=$gi_num_page;
       $st_requete = "select distinct date_demande,ca.nom,idf_acte from demandes_adherent da join commune_acte ca on (da.idf_commune=ca.idf) join adherent on (da.idf_adherent=adherent.idf) where adherent.ident=\"$gst_ident\" and idf_type_acte=$i_idf_type_acte and year(date_demande)=$i_annee and month(date_demande)=$i_mois order by date_demande desc";
       //print("Req=$st_requete<br>");
       $a_liste_ddes = $connexionBD->sql_select_multiple($st_requete);       
       $i_nb_ddes = count($a_liste_ddes);
       if ( $i_nb_ddes>0)
       {
          $pagination = new PaginationTableau($_SERVER['PHP_SELF'],'num_page_ddes_adht',$i_nb_ddes,NB_LIGNES_PAR_PAGE,DELTA_NAVIGATION,array('Date de la demande','Commune','Date de l\'acte','Parties'));
          $a_actes = array();
          foreach ($a_liste_ddes as $a_dde)
          {
          ...
          foreach ($a_liste_ddes as $a_dde)
          {
            ...
            {
              $a_tableau_affichage[] = array($st_date_dde,$st_commune,'&nbsp;',"R&eacute;f&eacute;rence originale de l'acte modifi&eacute;e");
            }  
          } 
          $pagination->init_page_cour($gi_num_page);
          $pagination->affiche_entete_liste_select('DemandesAdherents');
