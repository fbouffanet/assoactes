...
$gst_requete_nb_liasses= "SELECT l.cote_liasse, n.nom_notaire, n.prenom_notaire, n.commentaire, ".
						 ...
						 "       f.nom as forme, n.idf_commune_etude, l.info_complementaires, l.in_liasse_consultable, ".
						 "       case when r.idf is null then 'non' else 'oui' end as liasse_releve ".
...
if ($i_nb_liasses>0) {
	...
	foreach ($a_liasses as $a_liasse) {
		...
		if( $i_idf_commune_etude=='' || $i_idf_commune_etude==0 )
			$a_tableau[] = array($st_cote_liasse,$st_nom_notaire,$st_prenom_notaire, $st_commentaire,'', $st_libelle_annees,$i_idf_forme_liasse, $st_detail); 
		else
			$a_tableau[] = array($st_cote_liasse,$st_nom_notaire,$st_prenom_notaire, $st_commentaire,$a_communes_acte[$i_idf_commune_etude], $st_libelle_annees, $i_idf_forme_liasse, $st_detail); 	
	}
