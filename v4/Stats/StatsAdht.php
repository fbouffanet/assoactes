...
if (isset($gi_idf_adherent ))
{
  switch ($gst_mode)
  {
    case 'VUE_STAT':
      ...
      print("<div class='TITRE'>Statistiques des demandes de l'adh&eacute;rent $st_adherent</div>");
    ...
    case 'VUE_DEMANDES_COMMUNE':
       ...
       $st_adherent=$connexionBD->sql_select1("select concat(prenom,' ',nom,'(',idf,')') from adherent where idf=$gi_idf_adherent");
       print("<div class='TITRE'>Demandes de l'adh&eacute;rent $st_adherent</div>");
      ...
      $gi_num_page = empty($_POST['num_page_ddes_adht']) ?  $i_session_num_page: (integer) $_POST['num_page_ddes_adht'];
       ...
       if ($i_nb_ddes>0)
       {
          $pagination = new PaginationTableau($_SERVER['PHP_SELF'],'num_page_ddes_adht',$i_nb_ddes,NB_LIGNES_PAR_PAGE,DELTA_NAVIGATION,array('Date de la demande','Adresse IP','Date de l\'acte','Parties'));
          $a_actes = array();
    ...
    case 'VUE_DEMANDES_MOIS_ANNNEE':
       ...
       if ($i_nb_ddes>0)
       {
          $pagination = new PaginationTableau($_SERVER['PHP_SELF'],'num_page_ddes_adht',$i_nb_ddes,NB_LIGNES_PAR_PAGE,DELTA_NAVIGATION,array('Date de la demande','Adresse IP','Commune','Date de l\'acte','Parties'));
          $a_actes = array();
          foreach ($a_liste_ddes as $a_dde)
          {
