...
from modif_url mod_u join acte a on (mod_u.idf_acte=a.idf) where mod_u.idf_modif=$pi_idf_modif";
    ...
    {
       case IDF_NAISSANCE: list($st_description,$st_commentaires)=description_naissance($pconnexionBD,$a_profession,$i_idf_acte);
         $st_releve_acte.="$st_description\n";
         $st_releve_acte.="$st_commentaires";
      break;
       case IDF_DECES: list($st_description,$st_commentaires)=description_deces($pconnexionBD,$a_profession,$a_commune_personne,$i_idf_acte); 
         $st_releve_acte.="$st_description\n";
         $st_releve_acte.="$st_commentaires";
       break;
       default: list($st_description_acte,$st_permalien)=description_mariage_ou_divers($pconnexionBD,$a_type_acte,$a_profession,$a_commune_personne,$i_idf_acte);
                 $st_releve_acte.= $st_description_acte;
    }
    print("<fieldset>");
...
function valide_modification($pconnexionBD,$pi_idf_modif)
{
    ...
    list($st_date,$st_type,$st_commune,$st_parties)=$pconnexionBD->sql_select_liste("select a.date,ta.nom,ca.nom,group_concat(concat(p.patronyme,' ',p.prenom) SEPARATOR ' X ') from modif_url mod_url join acte a on (mod_url.idf_acte=a.idf) join commune_acte ca on (a.idf_commune=ca.idf) join type_acte ta on (a.idf_type_acte=ta.idf) join personne p on (a.idf=p.idf_acte) where p.idf_type_presence=".IDF_PRESENCE_INTV." and idf_modif=$pi_idf_modif group by mod_url.idf_modif,a.date,ta.nom,ca.nom");
    ...
    $st_requete = "update acte set url='$st_permalien' where idf=$i_idf_acte";
...
function affiche_refus($pconnexionBD,$pi_idf_modif)
{
  ...
  if (get_magic_quotes_gpc())
  {
    ...
    $st_motif_refus = isset($_POST['motif_refus']) ? $_POST['motif_refus']: 'ERREUR';
  $st_motif_refus= mysql_escape_string($st_motif_refus);
  $st_requete = "update modif_url set idf_validateur=$i_idf_validateur,date_validation=now(),statut='R',motif_refus='$st_motif_refus' where idf_modif=$pi_idf_modif";
  $pconnexionBD->execute_requete($st_requete);
  $st_entete  = 'MIME-Version: 1.0' . "\r\n";    
  $st_entete .= 'Content-type: text/html; charset=cp1252' . "\r\n";
  $st_entete .= "From: $st_prenom_valideur $st_nom_valideur <$st_email_valideur>\n";
