...
function menu_modifier($pconnexionBD,$pi_idf_photo,$pa_communes,$pa_collections,$pa_adherents)
{
   ...
   print("<form  action=\"".$_SERVER['PHP_SELF']."\" method=\"post\" id=\"edition_photos\">");
...
function charge_photos($pconnexionBD)
{
   ...
   while (( list($i_num_paroisse,$st_nom_paroisse,$st_fourchette,$st_nature,$i_taille,$i_nbre,$st_auteur,$i_idf_adh,$st_date_prise) = fgetcsv($pf, 1000,';')) !== FALSE)
   {
      ...
      {
         print("<div class=\"alert alert-danger\">Impossible de trouver une commune($i_code_insee,$i_num_paroisse)</div>");
      }
   }
   fclose($pf);
   if (count($a_lignes)>0)
   {
    ...
    try
    {
      $pconnexionBD->execute_requete($st_requete);
       ...
       die('<div class=\"alert alert-danger\">Chargement photos impossible: ' . $e->getMessage()."$st_requete</div>");
    }
   }	
   unlink($st_fich_dest);
