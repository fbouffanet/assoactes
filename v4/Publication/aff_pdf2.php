...
publication, faite sans le consentement de l&rsquo;A.G. C. 16, est illicite et constitue une contrefa&ccedil;on.<br />
...
function Mois_Annee ()  // PL 23/04/2014  Function pour affichage du mois en fran�ais
{
   $mois = array('', 'Janvier', 'F&eacute;vrier', 'Mars', 'Avril', 'Mai', 'Juin', 'Juillet', 'Ao&ucirc;t', 'Septembre', 'Octobre', 'Novembre', 'D&eacute;cembre');
   $mois_numero = date("n");   // ou $mois_numero = date("n");    m donne 01 � 12, n donne 1 � 12
...
function Footer()
  {
   if($this->PageNo()==1)
    ...
    // PL 23/04/2014 remplacement affichage $today par appel la function Mois_Annee pour mois en fran�ais
    //	$today = date("M-y");
    $today = Mois_Annee();
    $titreBP = "�".$today." Association G�n�alogique de la Charente  - Page ";
 ...
 switch ($type_actes_nimegue)
{
	...
	case "D"://selection sur les d�c�s
...
while ($data=$connexionBD->ligne_suivante_resultat($req))
   {
      ...
      if ($ligne == 2)
	      ...
	      //$pdf->Ln(10);// Saut de ligne
    	   $titre2 = 'Ann�es '.$datemini.' � '.$datemaxi.'  soit '.$nbractes.' actes';
		   ...
		   $titre3 = "Par ordre alphab�tique";
		   $pdf->SetFont('Times','',16);// Police Times gras 12
		   $pdf->Cell(0,10,$titre3,0,1,'C');//Tri par ordre alphab�tique
    	   $pdf->SetFont('Times','',8);
		   $pdf->Ln(10);// Saut de ligne
  ...
  case "M":// $type_actes_nimegue = M
   ...
   // info epoux
	...
	//if ($data[data13]!=''or 0) {$datenaiss1 = "";} else {$datenaiss1 = " n� le : ".$data[data13]." ";}
  ...
  // info Intervenant1
	...
	if ($data[16]!=''or empty($data[16])) {$datenaiss1 = "";} else {$datenaiss1 ="n&eacute;(e) le : ".$data[16]." ";}//$datenaiss
	...
	if (empty($commentaireEp)){} else {$l1= $l1."  - ".$commentaireEp."\n";}
	if (empty($data[39])){} else {$l1= $l1."  - "."Ex &eacute;pouse : ".$data[39]." ".$data[40]." ".$data[41]."\n";}
...
print $nom_fichier;
ob_end_clean(); // D�truit les donn�es du tampon de sortie et �teint la temporisation de sortie
