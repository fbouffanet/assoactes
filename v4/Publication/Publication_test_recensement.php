...
switch ($gst_mode) {
  ...
  case 'EXPORTV3':
    ...
    switch ($gc_idf_type_acte) {
      ...
      ";
      //=================================================
      $sqltmp="SELECT p.idf_acte, p.idf, 'R' as Sigle, f.nom AS Commune, a.annee AS Annee_Recensement, CAST(SUBSTRING(a.commentaires,INSTR(a.commentaires,'N de page:')+12,3) AS INT) AS Page, SUBSTRING(a.commentaires,INSTR(a.commentaires,'Quartier')+9,10) AS Quartier, SUBSTRING(a.commentaires,INSTR(a.commentaires,'Nom de la Rue:')+14,10) AS Rue, CAST(SUBSTRING(a.commentaires,INSTR(a.commentaires,'N° maison:')+10,3)AS INT) AS Maison, CAST(SUBSTRING(a.commentaires,INSTR(a.commentaires,'N° ménage:')+10,3)AS INT) AS Menage, p.patronyme AS Nom, IFNULL(prenom.libelle,'') AS Prenom, IFNULL(p.commentaires,''), IFNULL(p.age,'') AS Age, RIGHT(p.date_naissance,4) AS Annee°, c.nom AS Lieu°, d.nom AS Profession FROM personne p LEFT JOIN prenom ON (p.idf_prenom=prenom.idf) JOIN commune_personne c ON (p.idf_origine=c.idf) JOIN profession d ON (p.idf_profession=d.idf) JOIN acte a ON (p.idf_acte=a.idf) JOIN commune_acte f ON (a.idf_commune=f.idf) WHERE a.idf_commune= $gi_idf_commune_acte AND a.idf_source=$gi_idf_source AND a.idf_type_acte= $gc_idf_type_acte";
      //============================================================================//
    ...
    if (filesize($st_export_nimv3) == 0) {
      ...
      print('<form action="aff_pdf _recemsement.php" method="post">');
      print('<p>');
      if ($menuDIV == "O") {
