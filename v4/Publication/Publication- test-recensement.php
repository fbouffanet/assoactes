...
function export_recensement($pconnexionBD,$pi_idf_source,$pi_idf_commune_acte,$pc_idf_type_acte,$pa_liste_personnes,$pa_liste_actes,$pf)
{
   ...
   ifnull(p.age,'') as Age, right(p.date_naissance,4) as Annee°, 
   c.nom as Lieu°, 
   ifnull(p.commentaires,'') as Observation, 
   a.url as Lien 
   b.idf_nom as Commune
      from 
   ...
   join commune_acte b on (a.idf_commune=b.idf) 
   where 
   a.idf_commune=$pi_idf_commune_acte
   and a.idf_source=1 
   and a.idf_type_acte=$pc_idf_type_acte 
   order by 
   Année_Recensement ASC,
   Page ASC, 
   Maison ASC, 
   Ménage ASC";
