...
$copy="L’achat des tables ne donne pas droit &agrave copie ou reproduction.
Toute reproduction ou repr&eacutesentation int&eacutegrale, ou partielle, par quelque proc&eacuted&eacute que ce soit, des pages publi&eacutees dans la pr&eacutesente
publication, faite sans le consentement de l’A.G. C. 16, est illicite et constitue une contrefaçon.
Art. L. 122-4 et 5 L. 335-2 & s. du Code de la propri&eacutet&eacute intellectuelle.";
$today = date("M-y");
...
{
   $mois = array('', 'Janvier', 'F&eacutevrier', 'Mars', 'Avril', 'Mai', 'Juin', 'Juillet', 'Août', 'Septembre', 'Octobre', 'Novembre', 'D&eacutecembre');
   $mois_numero = date("n");   // ou $mois_numero = date("n");    m donne 01 &agrave 12, n donne 1 &agrave 12
...
function Footer()
  {
   if($this->PageNo()==1)
    ...
    {   //Pages suivantes
    $this->SetY(-15);// Positionnement &agrave 1,5 cm du bas
    $this->SetFont('Times','I',8);// Police Times italique 8

// PL 23/04/2014 remplacement affichage $today par appel la function Mois_Annee pour mois en français
//	$today = date("M-y");
   $today = Mois_Annee();
    $titreBP = "©".$today." Association G&eacuten&eacutealogique de la Charente  - Page ";
...
// Rajout PL *************************************************
// On r&eacutecup&eacutere les donn&eacutees date et nb d'actes dans le fichier txt
    $gst_repertoire_publication = $_SERVER['DOCUMENT_ROOT'].'/v4/Publication/telechargements';
 ...
 switch ($type_actes_nimegue)
{
	...
	$titre = "Bapt&ecircmes Naissances";
	//$titreHP = $titre." de ".utf8_encode($commune);
    $titreHP = $titre." de ".$commune;
	$pdf->titrehp = $titreHP;
	break;

	case "D"://selection sur les d&eacutec&egraves
	$titre = "D&eacutec&egraves S&eacutepulture";
	$titreHP = $titre." de ".$commune;
	break;
...
while ($data=$connexionBD->ligne_suivante_resultat($req))
   {
      ...
      if ($ligne == 2)
	      ...
	      //$pdf->Ln(10);// Saut de ligne
    	   $titre2 = 'Ann&eacutees '.$datemini.' &agrave '.$datemaxi.'  soit '.$nbractes.' actes';
		   ...
		   $titre3 = "Par ordre alphab&eacutetique";
		   $pdf->SetFont('Times','',16);// Police Times gras 12
		   $pdf->Cell(0,10,$titre3,0,1,'C');//Tri par ordre alphab&eacutetique
    	   $pdf->SetFont('Times','',8);
		   $pdf->Ln(10);// Saut de ligne
    	   ...
    	   //$pdf->Ln(40);
		   $pdf->SetY(-45);// Positionnement &agrave 1,5 cm du bas
    	   $pdf->Cell(20);
   ...
   case "N":// $type_actes_nimegue = N
	...
	$l1='';
  	if (empty($data[14])){} else {$l1= $l1."  - "."P&egravere   ".$data[14]."   ".$data[15]."   ".$data[16]."   ".$data[17]."\n";}
  	if (empty($data[18])){} else {$l1= $l1."  - "."M&egravere   ".$data[18]."   ".$data[19]."   ".$data[20]."   ".$data[21]."\n";}
 	if (empty($data[22])){} else {$l1= $l1."  - "."Par/T&eacutem1   ".$data[22]."   ".$data[23]."   ".$data[24]."\n";}
  	if (empty($data[25])){} else {$l1= $l1."  - "."Par/T&eacutem2   ".$data[25]."   ".$data[26]."   ".$data[27]."\n";}
  ...
  case "D":// $type_actes_nimegue = D
	...
	if ($data[13] !== "") {$datenaiss ="n&eacute(e) le : ".$data[13];} else {$datenaiss = "";}//$datenaiss = $data[data13]
	...
	if (empty($data[22])){} else {$l1= $l1."  - "."P&egravere   ".$data[22]."   ".$data[23]."   ".$data[24]."   ".$data[25]."\n";}
	if (empty($data[26])){} else {$l1= $l1."  - "."M&egravere   ".$data[26]."   ".$data[27]."   ".$data[28]."   ".$data[29]."\n";}
	if (empty($data[30])){} else {$l1= $l1."  - "."T&egravem1   ".$data[30]."   ".$data[31]."   ".$data[32]."\n";}
	if (empty($data[33])){} else {$l1= $l1."  - "."T&egravem2   ".$data[33]."   ".$data[34]."   ".$data[35]."\n";}
  ...
  case "M":// $type_actes_nimegue = M
   ...
   // info epoux
	...
	//if ($data[data13]!=''or 0) {$datenaiss1 = "";} else {$datenaiss1 = " n&eacute le : ".$data[data13]." ";}
	if ($data[13]!=''or empty($data[13])) {$datenaiss1 = "";} else {$datenaiss1 = " n&eacute le : ".$data[13]." ";}//$datenaiss
	...
	if (empty($data[20])){} else {$l1= $l1."   - "."P&egravere  : ".$data[20]." ".$data[21]." ".$data[22]." ".$data[23]."\n ";}
	if (empty($data[24])){} else {$l1= $l1."   - "."M&egravere  : ".$data[24]." ".$data[25]." ".$data[26]." ".$data[27]." \n";}

	//Info Epouse
	if (empty($data[30])) {$lieuorigine2 = "";} else {$lieuorigine2 = " Originaire de ".$data[30]." ";}  //$lieuorigine
	if ($data[31]!=''or empty($data[31])) {$datenaiss2 = "";} else {$datenaiss2 =" n&eacutee le : ".$data[31]." ";}//$datenaiss
	...
	if (empty($data[38])){} else {$l1= $l1."   - "."P&egravere  : ".$data[38]." ".$data[39]." ".$data[40]." ".$data[41]."\n";}
	if (empty($data[42])){} else {$l1= $l1."   - "."M&egravere  : ".$data[42]." ".$data[43]." ".$data[44]." ".$data[45]."\n";}
  	// T&eacutemoins
  	if (empty($data[46])){} else {$l1= $l1."    - "."T&eacutemoin 1  : ".$data[46]." ".$data[47]." ".$data[48]."\n ";}
	if (empty($data[49])){} else {$l1= $l1."    - "."T&eacutemoin 2  : ".$data[49]." ".$data[50]." ".$data[51]."\n ";}
  	if (empty($data[52])){} else {$l1= $l1."    - "."T&eacutemoin 3  : ".$data[52]." ".$data[53]." ".$data[54]."\n ";}
	if (empty($data[55])){} else {$l1= $l1."    - "."T&eacutemoin 4  : ".$data[55]." ".$data[56]." ".$data[57]."\n ";}
  ...
  // info Intervenant1
	...
	if ($data[16]!=''or empty($data[16])) {$datenaiss1 = "";} else {$datenaiss1 ="n&eacute le : ".$data[16]." ";}//$datenaiss
	...
	if (empty($info1)){} else {$l1= $l1."  - ".$info1."\n";}
	if (empty($commentaireEpx)){} else {$l1= $l1."  - ".$commentaireEpx."\n";}
	if (empty($data[20])){} else {$l1= $l1."  - "."Ex &eacutepouse : ".$data[20]." ".$data[21]." ".$data[22]."\n";}
	if (empty($data[23])){} else {$l1= $l1."  - "."P&egravere  : ".$data[23]." ".$data[24]." ".$data[25]." ".$data[26]."\n";}
	if (empty($data[27])){} else {$l1= $l1."  - "."M&egravere  : ".$data[27]." ".$data[28]." ".$data[29]." ".$data[30]."\n";}

	//Info Intervenant2
	if (empty($data[34])) {$lieuorigine2 = "";} else {$lieuorigine2 = "Originaire de ".$data[34]." ";}  //$lieuorigine
	if ($data[35]!=''or empty($data[35])) {$datenaiss2 = "";} else {$datenaiss2 ="n&eacutee le : ".$data[35]." ";}//$datenaiss
	...
	if (empty($commentaireEp)){} else {$l1= $l1."  - ".$commentaireEp."\n";}
	if (empty($data[39])){} else {$l1= $l1."  - "."Ex &eacutepoux : ".$data[39]." ".$data[40]." ".$data[41]."\n";}
	if (empty($data[42])){} else {$l1= $l1."  - "."P&egravere  : ".$data[42]." ".$data[43]." ".$data[45]." ".$data[44]."\n";}
	if (empty($data[46])){} else {$l1= $l1."  - "."M&egravere  : ".$data[46]." ".$data[47]." ".$data[49]." ".$data[48]."\n";}

  	// T&eacutemoins
  	if (empty($data[50])){} else {$l1= $l1."\n"."  - "."T&eacutemoin 1  : ".$data[50]." ".$data[51]."  ".$data[52]."\n";}
	if (empty($data[53])){} else {$l1= $l1."  - "."T&eacutemoin 2  : ".$data[53]." ".$data[54]."  ".$data[55]."\n";}
	if (empty($data[56])){} else {$l1= $l1."  - "."T&eacutemoin 3  : ".$data[56]." ".$data[57]."  ".$data[58]."\n";}
  	if (empty($data[59])){} else {$l1= $l1."  - "."T&eacutemoin 4  : ".$data[59]." ".$data[60]."  ".$data[61]."\n";}
...
switch ($type_actes_nimegue) { //Ajout du repertoire par Epouses ou Interv2
   ...
   case "M":
	$pdf->AddPage();
	$titre3 = "Par ordre alphab&eacutetique sur l'&eacutepouse";
	$pdf->SetFont('Times','',16);// Police Times gras 12
	$pdf->Cell(0,10,$titre3,0,1,'C');//Tri par ordre alphab&eacutetique
    $pdf->SetFont('Times','',8);
	//liste_epouses_m();
	...
	case "V":
	 $pdf->AddPage();
	 $titre3 = "Par ordre alphab&eacutetique sur l'intervenant 2";
	 $pdf->SetFont('Times','',16);// Police Times gras 12
	 $pdf->Cell(0,10,$titre3,0,1,'C');//Tri par ordre alphab&eacutetique
     $pdf->SetFont('Times','',8);
...
print $nom_fichier;
ob_end_clean(); // D&eacutetruit les donn&eacutees du tampon de sortie et &eacuteteint la temporisation de sortie
