...
$gst_requete_nb_actes= "select a.idf,p.patronyme,p.prenom,p.idf_type_presence,a.idf_type_acte,a.details_supplementaires,a.annee,a.mois,a.jour from `acte` a join `source` s on (a.idf_source=s.idf) join `personne` p on (p.idf_acte=a.idf) join commune_acte ca on (a.idf_commune=ca.idf)where s.publication_geneabank=1 and ";
...
if ($gst_prenom!='')
{
   ...
   $a_clauses[]="p.prenom ".$requeteRecherche->clause_droite_prenom($gst_prenom,1);
   $connexionBD->ajoute_params(array(":prenom1"=>$gst_prenom));
}
