...
$gst_requete_nb_actes= "select u.idf_acte,u.patronyme_epoux,p1.prenom,p1.idf_type_presence,u.patronyme_epouse,p2.prenom,p2.idf_type_presence,a.idf_type_acte,a.details_supplementaires,a.annee,a.mois,a.jour from `union` u  join `acte` a on (u.idf_acte=a.idf) join `source` s on (a.idf_source=s.idf) join `personne` p1 on (u.idf_epoux=p1.idf) join `personne` p2 on (u.idf_epouse=p2.idf) join commune_acte ca on (a.idf_commune=ca.idf) where s.publication_geneabank=1 and ";
...
if (!empty($gst_prenom_epx))
{
    ...
    $a_clauses[]="p1.prenom ".$requeteRecherche->clause_droite_prenom($gst_prenom_epx,1);
    $connexionBD->ajoute_params(array(":prenom1"=>$gst_prenom_epx));
}
if (!empty($gst_prenom_epse))
{
    $gst_prenom_epse  = str_replace('*','%', $gst_prenom_epse);
    $a_clauses[]="p2.prenom ".$requeteRecherche->clause_droite_prenom($gst_prenom_epse,2);
    $connexionBD->ajoute_params(array(":prenom2"=>$gst_prenom_epse));
}
...
if ($i_nb_actes>0)
{
  ...
  $gst_requete_intv = "select idf_acte,personne.idf,sexe,patronyme, prenom,idf_type_acte,idf_pere,idf_mere from personne join acte on (idf_acte=acte.idf) where idf_acte in (".join(',',$a_idf_acte).") and idf_type_presence=".IDF_PRESENCE_INTV. " order by idf_acte,personne.idf";
  $etape_prec = getmicrotime();
