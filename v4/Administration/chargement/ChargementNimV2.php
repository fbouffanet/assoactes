...
class ChargementNimV2 {
   ...
   public function __construct ($pconnexionBD,$pst_parametres_load_data) {
      $this->i_nb_actes = 0;
      $this->a_deja_existants = array();
      $this->st_parametres_load_data = $pst_parametres_load_data;
   }
   ...
   * @return boolean le chargement s'est-il bien passé ? 
   */ 
   function charge_mariages($pst_fichier,$pi_idf_commune,$pi_idf_source,$pi_idf_releveur,$pst_rep_trav,$pa_liste_mariages_existants)
   {
      ...
      {         
         ...
         //Corrige une erreur dans les relevés : Si l'ancient conjoint est uniquement "Veuve", on considère que c'est un commentaire sur l'épouse. Aucun ancien conjoint ne sera créé
         ...
         if (!isset($pa_liste_mariages_existants[strval($st_date)][strval($st_nom_epx)][strval($st_prn_epx)][strval($st_nom_epse)][strval($st_prn_epse)]))
         {
            ...
            if ($st_cjt_epx!= '')
            {         
              //Corrige une erreur dans les relevés : Si l'ancien conjoint est uniquement "veuf", on considère que c'est un commentaire sur l'époux. Aucun ancien conjoint ne sera créé
            ...
            if ($st_cjt_epse!= '')
            {
               //Corrige une erreur dans les relevés : Si l'ancient conjoint est uniquement "veuf", on considère que c'est un commentaire sur l'époux. Aucun ancien conjoint ne sera créé
               if (preg_match("/^\(*Veuve\)*$/i",$st_cjt_epse) || $st_cjt_epse=='?' || $st_cjt_epse=='!')
                  $st_cmt_epse .= " Veuve";
               else
               { 
 ...
 */ 
function charge_divers($pst_fichier,$pi_idf_commune,$pi_idf_source,$pi_idf_releveur,$pst_rep_trav,$pa_liste_divers_existants)
{
   global $gst_serveur_bd,$gst_utilisateur_bd,$gst_mdp_utilisateur_bd,$gst_nom_bd;
   $connexionBD = ConnexionBD::singleton($gst_serveur_bd,$gst_utilisateur_bd,$gst_mdp_utilisateur_bd,$gst_nom_bd);
   $type_acte = TypeActe::singleton($connexionBD);
 ...
 * @return boolean : a reussi ou pas  
 */ 
function charge_naissances($pst_fichier,$pi_idf_commune,$pi_idf_source,$pi_idf_releveur,$pst_rep_trav,$pa_liste_naissances_existantes)
{
   global $gst_serveur_bd,$gst_utilisateur_bd,$gst_mdp_utilisateur_bd,$gst_nom_bd;
   $connexionBD = ConnexionBD::singleton($gst_serveur_bd,$gst_utilisateur_bd,$gst_mdp_utilisateur_bd,$gst_nom_bd);
   $type_acte = TypeActe::singleton($connexionBD);
 ...
 * @return boolean : a reussi ou pas  
 */ 
function charge_deces($pst_fichier,$pi_idf_commune,$pi_idf_source,$pi_idf_releveur,$pst_rep_trav,$pa_liste_deces_existants)
{
   global $gst_serveur_bd,$gst_utilisateur_bd,$gst_mdp_utilisateur_bd,$gst_nom_bd;
   $connexionBD = ConnexionBD::singleton($gst_serveur_bd,$gst_utilisateur_bd,$gst_mdp_utilisateur_bd,$gst_nom_bd);
   $type_acte = TypeActe::singleton($connexionBD);
