...
class CommunePersonne {
   ...
   public function ajoute($pst_commune) {
     if ($pst_commune != '' && !array_key_exists(strval(trim($pst_commune)),$this->a_idf_par_commune ) && !in_array(strval(trim($pst_commune)),$this->a_commune_a_creer ) )
         $this->a_commune_a_creer[]=trim($pst_commune);
   }
   
   public function sauve() {
     $a_params_precs=$this->connexionBD->params();
	 $a_commune_a_creer = array();
	 if (count($this->a_commune_a_creer)>0)
     {
	   $st_requete = "insert INTO `commune_personne` (nom) values ";
       $a_colonnes = array();
	   $i=0;
	   foreach ($this->a_commune_a_creer as $st_elem)
       {
